...
export default function FooterSection() {
  return (
    <footer className="relative w-full bg-[#09090b] border-t border-white/5 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-[5%] py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
          ...
          <div className="space-y-4">
            <h2 className="text-white font-semibold">Wakin Dev_</h2>
          ...
          {/* Navigation */}
          <div>
            <h3 className="text-neutral-400 text-xs mb-4">Navigation</h3>
            {navLinks.map((link) => (
              <button
                key={link.label}
                onClick={() => scrollTo(link.href)}
                className="block text-neutral-500 hover:text-purple-400 text-sm"
              >
                {link.label}
              </button>
            ))}
        ...
        <div className="text-center text-neutral-600 text-xs">
          © {new Date().getFullYear()} Wacky D. Hojilla
        </div>
